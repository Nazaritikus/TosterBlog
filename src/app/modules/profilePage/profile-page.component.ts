...
export class ProfilePageComponent implements OnInit, OnDestroy, ComponentCanDeactivate {
    ...
    this.userMainInfoSub = this.userServices.updateClientProfileData({[updateUser.fbId]: updateUser}, this.localId).subscribe(
      () => this.dialog.open(
        DialogMessagesComponent, {
          data: {message: 'Your Main Information has been updated.', title: 'WooHoo!'}
        }),
      () => this.dialog.open(
  ...
  async updateUserPhoto(mainPhoto: boolean) {
    ...
    const photoPath = `${localStorage.getItem('fb-user')}/${mainPhoto ? 'mainPhoto' : 'coverPhoto'}_${new Date().getTime()}`;
  ...
  ngOnDestroy(): void {
    ...
    if (this.userMainInfoSub) {
      this.userMainInfoSub.unsubscribe();
    }
    if (this.photoUploadSub) {
      this.photoUploadSub.unsubscribe();
    }
    if (this.aboutInfoSub) {
      this.aboutInfoSub.unsubscribe();
    }
  }
}
